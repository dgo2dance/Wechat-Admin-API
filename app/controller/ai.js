...
class AiController extends Controller {
  constructor(ctx) {
    ...
    this.createTransfer = ctx.app.Joi.object().keys({
      ...
      startKey: ctx.app.Joi.string().required().label(ctx.__('startKey')),
      endKey: ctx.app.Joi.string().required().label(ctx.__('endKey'))
    }).unknown();
  ...
  async update() {
    ...
    await service.ai.update( filterData)
    // 设置响应内容和响应状态码
    ctx.helper.success({ctx,res:true})
  }
  ...
  async show() {
    const { ctx, service } = this
    // 调用 Service 进行业务处理
    const res = await service.ai.show()
    // 设置响应内容和响应状态码
